...
const generateRelatedData = (originalCategory: string) => {
  ...
  categories.forEach((category) => {
    const strength = Math.floor(Math.random() * 4) + 7; // Random strength between 7-10
...
const SelfMapVisualization: React.FC<SelfMapProps> = ({ data }) => {
  ...
  const createVisualization = (
    ...
    const xAxis = d3.axisBottom(xScale).tickSize(-5).tickPadding(10);
    const yAxis = d3.axisLeft(yScale).tickSize(-5).tickPadding(10);
    ...
    points
      ...
      .style(
        'fill',
        `url(#point-gradient-${isSecondary ? 'secondary' : 'primary'})`
      );
    ...
    points
      ...
      .on('mouseover', function (event, d) {
        const tooltip = d3
          ...
          .attr(
            'class',
            'absolute bg-black bg-opacity-90 text-white p-4 rounded-lg pointer-events-none opacity-0 transition-opacity duration-300 text-sm max-w-xs shadow-lg border border-gray-700'
          );
        ...
        tooltip
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full" style="background: ${colorScale(
                d.name
              )}"></div>
            ...
            ${
              d.details.Title
                ? `<div class="text-gray-300">Role: ${d.details.Title}</div>`
                : ''
            }
            ${
              d.details.Beliefs
                ? `<div class="text-gray-300">Beliefs: ${d.details.Beliefs}</div>`
                : ''
            }
            ${
              d.details.Style
                ? `<div class="text-gray-300">Style: ${d.details.Style}</div>`
                : ''
            }
          </div>
        `
      ...
      .on('click', function (event, d) {
        if (!isSecondary) {
          ...
          setTimeout(() => {
            createVisualization(
              secondaryChartRef,
              relatedData[Object.keys(relatedData)[0]],
              true
            );
          }, 100);
        }
    ...
    svg
      ...
      .style('fill', '#333')
      .text(
        isSecondary ? `${selectedCategory} Details` : 'Personal Identity Map'
      );
        ...
        {showInfo && (
          <div className="absolute top-14 right-4 w-64 bg-white p-4 rounded-lg shadow-xl border border-gray-200 z-10 text-sm">
            ...
            <ul className="space-y-2 text-gray-600">
              <li>â?¢ Hover over points to see details</li>
              <li>â?¢ Click points to explore sub-categories</li>
              <li>â?¢ Points size indicates strength</li>
              <li>â?¢ Lines show relationships</li>
            </ul>
          </div>
        )}
